...
\begin{frame}[fragile]
{Pointers are a big overhead for nodes with small sizes}
...
\begin{lstlisting}
struct node {
  // Uses an index instead of pointer.
  std::uintXY_t next;
  // Small value type accurs pretty often.
  std::uintXY_t info;
};
\end{lstlisting}
\begin{block}{Thoughts}
\begin{itemize}
\item On a 64 bits system the struct size is 16 bytes.
...
\item In the general case however we cannot reduce
the {value\_type} at will.
\end{itemize}
\end{block}
