...
\begin{column}{0.47\textwidth}
\begin{lstlisting}
// Allocator configured to chuncks of 256
// elements.
using alloc_type =
...
\begin{tikzpicture}[scale=1.0]
\node[anchor=south, draw,node distance=0cm, fill=nodecontc,shape=rectangle,minimum height=7.00cm  ,minimum width=1.0cm]
(node0) at (0cm,0cm) {};
\node[anchor=south,draw,node distance=0cm, fill=bothcontc,shape=rectangle,minimum height=0.43cm  ,minimum width=1.0cm]
(node1) at (1cm,0.0cm) {};

\node[] at (0cm,4.5cm) {\color{black} Traversal time};
\filldraw[nodecontc] (0cm,4.0cm) circle (0.08cm) node[right]
{\color{black} Pointer chasing};
\filldraw[bothcontc] (0cm,3.5cm) circle (0.08cm) node[right]
...
\begin{columns}
    ...
    \begin{column}{0.47\textwidth}
         \begin{block}{Why so much speed up?}
           ...
           \item But times more data in the cache.
           \end{itemize}
         \end{block}
    \end{column}
