...
// C++ style. Calls constructors.
auto* p = new int[10];
delete [] p;

// Smart pointers. Proper way to handle
// allocated memory dynamicaly.
...
\begin{column}[t]{0.48\textwidth}
Inside STL containers allocators are used instead\\
\vspace{1cm}
\begin{lstlisting}
...
alloc.construct(p); // Calls constructors
...
allocations with compile time sizes.
\end{block}
...
For runtime allocation sizes I will use {\bf \color{alertc} array allocation}.
\end{block}
\end{column}
\end{columns}
\end{frame}
...
\fill[nodecontc]  (180:2.5cm) -- (180: 3cm) arc (180:360:3cm) -- (360:2.5cm) arc (360:180:2.5cm);
\fill[bothcontc]   (30:2.5cm) -- (30:3cm) arc (30:180:3cm)  -- (180:2.5cm) arc (180:30:2.5cm);
\fill[arraycontc] (0:2.5cm) -- (0:3cm) arc (0:30:3cm) -- (30:2.5cm) arc (30:0:2.5cm);
...
\begin{enumerate}
\item<alert@1> Get a block of memory with $N$ nodes at once.
\item<alert@2> Link them as a stack.
\item<alert@3> Repeat if more nodes are inserted.
\end{enumerate}
\end{block}
...
\begin{tikzpicture}[scale=1.0, rounded corners]
   ...
   \uncover<2-> {
      \marrow (node0.north) to [bend right=-45] (node1.north);
      \marrow (node1.north) to [bend right=-45] (node2.north);
      \marrow (node2.north) to [bend right=-45] (node3.north);
      \marrow (node3.north) to [bend right=-45] (node4.north);
      \marrow (node4.north) to [bend right=-45] (node5.north);
      \marrow (node5.north) to [bend right=-45] (node6.north);
   }
   ...
   \uncover<3-> {
      ...
      \marrow (node7.north) to [bend right=-45] (node8.north);
      \marrow (node8.north) to [bend right=-45] (node9.north);
      \marrow (node9.north) to [bend right=-45] (node10.north);
      \marrow (node10.north) to [bend right=-45] (node11.north);
      \marrow (node11.north) to [bend right=-45] (node12.north);
      \marrow (node12.north) to [bend right=-45] (node13.north);
   }
\end{tikzpicture}
...
\begin{block} {Can we avoid pointers chasing?}
\begin{enumerate}
