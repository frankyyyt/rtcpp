...
\lstset{
  ...
  backgroundcolor=\color{white!85!black},   % choose the background color; you must add \usepackage{color} or \usepackage{xcolor}
  basicstyle=\ttfamily\footnotesize,        % the size of the fonts that are used for the code
  columns=fullflexible
  breakatwhitespace=false,         % sets if automatic breaks should only happen at whitespace
  breaklines=true,                 % sets automatic line breaking
  keywordstyle=\color{blue!50!black},       % keyword style
...
\section{Common allocation goals and strategies}
%____________________________________________________________
...
\begin{tikzpicture}[scale=1.0, rounded corners]
   ...
   \uncover<2> {
      \marrow (node2.north) to [bend right=-25] (node6.north);
      \marrow (node6.south) to [bend right=-25] (node4.south);
      \marrow (node4.south) -- (node11.north);
      \marrow (node11.north) to [bend right=-25] (node9.north);
      \marrow (node7.south) to [bend right=-25] (node12.south);
