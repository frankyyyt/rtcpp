...
{ Marcelo Zimbres (\href{mzimbres@gmail.com} {mzimbres@gmail.com})\\
\vspace{1cm}
Code: \href{https://github.com/mzimbres/rtcpp}
{https://github.com/mzimbres/rtcpp} \\
\vspace{1cm}
Proposal: \href{http://open-std.org/jtc1/sc22/wg21/docs/papers/2016/p0310r0.pdf}
...
\fill[arraycontc] (0:2.5cm) -- (0:3.3cm) arc (0:57:3.3cm) -- (57:2.5cm) arc (57:0:2.5cm);
\fill[bothcontc]  (57:2.5cm) -- (57:3.3cm) arc (57:253:3.3cm) -- (253:2.5cm) arc (253:57:2.5cm);
\fill[nodecontc]  (253:2.5cm) -- (253:3.3cm) arc (253:343:3.3cm) -- (343:2.5cm) arc (343:253:2.5cm);
\fill[memC]       (343:2.5cm) -- (343:3.3cm) arc (343:360:3.3cm) -- (360:2.5cm) arc (360:343:2.5cm);

\filldraw[white] (-1.5cm, 1.00cm) circle (0.08cm) node[right] {\color{black} Jobs};
\filldraw[arraycontc] (-1.5cm, 0.50cm) circle (0.08cm) node[right] {\color{black} Edorado Institute};
\filldraw[bothcontc]  (-1.5cm, 0.00cm) circle (0.08cm) node[right] {\color{black} Numerical Offshore Tank};
\filldraw[nodecontc]  (-1.5cm,-0.50cm) circle (0.08cm) node[right] {\color{black} Robert Bosch GmbH};
\filldraw[memC]       (-1.5cm,-1.00cm) circle (0.08cm) node[right] {\color{black} Hess Cash Systems};
\end{tikzpicture}
\end{column}

\begin{column}[c]{0.45\textwidth}
\begin{tikzpicture}[scale=1.0]
\fill[arraycontc] (0:2.5cm) -- (0:3.3cm) arc (0:125:3.3cm) -- (125:2.5cm) arc (125:0:2.5cm);
\fill[bothcontc]  (125:2.5cm) -- (125:3.3cm) arc (125:187:3.3cm) -- (187:2.5cm) arc (187:125:2.5cm);
\fill[nodecontc]  (187:2.5cm) -- (187:3.3cm) arc (187:360:3.3cm) -- (360:2.5cm) arc (360:187:2.5cm);

\filldraw[white]     (-1.7cm,1.00cm) circle (0.08cm) node[right] {\color{black} Education};
\filldraw[arraycontc](-1.7cm,0.50cm) circle (0.08cm) node[right] {\color{black} Bsc Physics};
\filldraw[bothcontc] (-1.7cm,-0.00cm) circle (0.08cm) node[right] {\color{black} Msc Physics (Gravitation)};
\filldraw[nodecontc] (-1.7cm,-0.50cm) circle (0.08cm) node[right] {\color{black} PhD Astrophisics};
\end{tikzpicture}
\end{column}
\end{columns}
\end{frame}
...
\begin{enumerate}
\item<alert@1> Get a continuous block of memory with enough space for $n$ nodes.
...
\begin{block}{Shouldn't need this if compile time information had been used.}
\begin{itemize}
...
\begin{block}{Node allocation is a building block}
  ...
  proper support in the STL. Now lets us see how we can get it in the STL and what further
  benefits it brings.
\end{block}
\end{column}
\begin{column}{0.10\textwidth}
...
\node[] at (3cm,4.5cm) {\color{black} Traversal time};
...
\end{lstlisting}
\begin{block}{Thoughts}
Reducing further to \texttt{std::uint16\_t} won't
make any difference
since the spared space will be wasted on padding due
to \texttt{std::string} alignment.
\end{block}
...
\item Four bytes are wasted on padding.
\item Using 32 bits pointers may not be an option.
\item Even if it were, 16 bits are usually enough.
\end{itemize}
\end{block}
...
(node2) at (2cm,0.0cm) {};
\node[anchor=south,draw,node distance=0cm, fill=purple,shape=rectangle,minimum height=1.167cm  ,minimum width=1.0cm]
(node3) at (3cm,0.0cm) {};
...
\end{lstlisting}
\begin{block}{Thoughts}
Pretty useful if the range of values $[0, 65536]$
is enough.
\end{block}
